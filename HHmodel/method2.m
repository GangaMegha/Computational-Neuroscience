...
freq = zeros(1, 200);
I = zeros(1,200);
end_val = zeros(1,200); % holds the location of last significant peak corresponding to each current value
I1 = 0;
I2 = 0;
I3 = 0;
k = 1;
...
for ImpCur = 0:0.0005:1
    ...
    % For finding the location of last significant peak => 75% of highest peak
    for j=1:1:length(peaks)
        if j~=1
            if (peaks(j-1))> 75/100*peaks(1)
                end_val(k) = locs(j);
            else
                break
            end
     ...
     if ImpCur < 0.0224
         I1 = ImpCur;
     elseif freq(k-1)~=0
         if (t(end))<(t(end_val(k)) + 20/freq(k-1))% last significant peak occurs before last time  => Finite no. of peaks
             I2 = ImpCur;
         else 
             I3 = ImpCur;
         end
